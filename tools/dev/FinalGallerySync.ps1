...
foreach ($mod in $Modules) {
    ...
    if (Test-Path $psd1Path) {
        ...
        # 1. Bump version to 1.5.0.1
        $content = $content -replace "ModuleVersion\s*=\s*'1.5.0.0'", "ModuleVersion = '1.5.0.1'"
...
foreach ($mod in $Modules) {
    Write-Output ">>> Uploading $($mod.Name) v1.5.0.1..." -ForegroundColor Cyan
