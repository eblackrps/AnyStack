...
foreach ($mod in $Modules) {
    ...
    # 3. Check metadata
    $missingMeta = $false
    if ($manifestContent -notmatch "ModuleVersion\s*=\s*'1\.7\.5'") { $missingMeta = $true; Write-Error "ModuleVersion not 1.7.5" }
...
if ($failed) {
    ...
    Write-Host "Validation PASSED. All 28 modules v1.7.5 are ready for publication." -ForegroundColor Green
    exit 0
}
