# VCF.SnapshotManager

**Part of the [AnyStack Enterprise Module Suite](https://github.com/eblackrps/AnyStack) · MIT License**

Snapshot lifecycle management. Cleans up orphaned snapshots and optimizes snapshot chains to prevent performance degradation.

## Requirements

- PowerShell 7.2+
- VCF.PowerCLI 9.0+
- vSphere 8.0 U3 / VCF 5.1+
- AnyStack.vSphere (connection management)

## Installation

```powershell
# Install the full AnyStack suite
Install-Module -Name AnyStack -Scope CurrentUser

# Or install this module individually
Install-Module -Name VCF.SnapshotManager -Scope CurrentUser
```

## Cmdlets

### `Clear-AnyStackOrphanedSnapshots`
Removes snapshots that exist in the VMDK chain but are not registered in the vCenter snapshot tree.

### `Optimize-AnyStackSnapshots`
Consolidates snapshot chains and removes snapshots exceeding a configurable age or count threshold.

## Usage

```powershell
Import-Module AnyStack.vSphere
Connect-AnyStackServer -Server 'vcenter.yourenv.local'

Import-Module VCF.SnapshotManager
```

All cmdlets support `-Verbose`, `-ErrorAction`, and `-WhatIf` where applicable.

## Links

- [Full module suite on GitHub](https://github.com/eblackrps/AnyStack)
- [PowerShell Gallery](https://www.powershellgallery.com/packages/VCF.SnapshotManager)
- [anystackarchitect.com](https://www.anystackarchitect.com)
 
