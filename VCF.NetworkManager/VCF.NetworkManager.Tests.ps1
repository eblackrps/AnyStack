...
Describe "VCF.NetworkManager Suite" {
    ...
    Context "New-AnyStackVlan" {
        ...
        It "Should be callable without throwing a syntax error" {
            { New-AnyStackVlan -Server 'MockVC' -PortGroupName 'PG01' -VlanId 100 -DvsName 'DVS01' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Set-AnyStackVlanTag" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Set-AnyStackVlanTag -Server 'MockVC' -PortGroupName 'PG01' -NewVlanId 200 -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
}
