...
Describe "VCF.LogIntelligence Suite" {
    ...
    Context "Clear-AnyStackStaleLogs" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Clear-AnyStackStaleLogs -Server 'MockVC' -HostName 'esxi1' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Get-AnyStackHostLogBundle" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Get-AnyStackHostLogBundle -Server 'MockVC' -HostName 'esxi1' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Set-AnyStackSyslogServer" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Set-AnyStackSyslogServer -Server 'MockVC' -HostName 'esxi1' -SyslogServer 'udp://syslog.stub:514' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
