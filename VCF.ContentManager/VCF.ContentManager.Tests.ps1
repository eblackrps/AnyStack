...
Describe "VCF.ContentManager Suite" {
    ...
    Context "Get-AnyStackLibraryItem" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Get-AnyStackLibraryItem -Server 'MockVC' -LibraryName 'TestLib' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "New-AnyStackVmTemplate" {
        ...
        It "Should be callable without throwing a syntax error" {
            { New-AnyStackVmTemplate -Server 'MockVC' -VmName 'vm01' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    ...
    Context "Sync-AnyStackContentLibrary" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Sync-AnyStackContentLibrary -Server 'MockVC' -LibraryName 'TestLib' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
}
