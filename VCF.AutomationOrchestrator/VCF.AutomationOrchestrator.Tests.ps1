...
Describe "VCF.AutomationOrchestrator Suite" {
    ...
    Context "New-AnyStackScheduledSnapshot" {
        ...
        It "Should be callable without throwing a syntax error" {
            { New-AnyStackScheduledSnapshot -Server 'MockVC' -VmName 'vm01' -SnapshotName 'snap01' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    ...
    Context "Sync-AnyStackAutomationScripts" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Sync-AnyStackAutomationScripts -Server 'MockVC' -LocalScriptPath 'C:\Windows\Temp' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
}
