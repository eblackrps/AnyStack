...
Describe "VCF.LifecycleManager Suite" {
    ...
    Context "Start-AnyStackHostRemediation" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Start-AnyStackHostRemediation -Server 'MockVC' -ClusterName 'c1' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Test-AnyStackCompliance" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Test-AnyStackCompliance -Server 'MockVC' -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
}
