...
Describe "VCF.StorageAdvanced Suite" {
    ...
    Context "Add-AnyStackNvmeInterface" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Add-AnyStackNvmeInterface -Server 'MockVC' -HostName 'esxi1' -ControllerAddress '192.168.1.1' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    ...
    Context "Remove-AnyStackNvmeInterface" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Remove-AnyStackNvmeInterface -Server 'MockVC' -HostName 'esxi1' -ControllerAddress '192.168.1.1' -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Set-AnyStackNvmeQueueDepth" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Set-AnyStackNvmeQueueDepth -Server 'MockVC' -HostName 'esxi1' -QueueDepth 32 -Confirm:$false -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
    Context "Test-AnyStackNvmeConnectivity" {
        ...
        It "Should be callable without throwing a syntax error" {
            { Test-AnyStackNvmeConnectivity -Server 'MockVC' -ErrorAction SilentlyContinue } | Should -Not -Throw
        }
    }
}
